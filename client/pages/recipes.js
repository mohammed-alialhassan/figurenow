...
export default function Recipes() {
    ...
    const files = [
        ...
        {
            title:'Jerk Chicken with Carribean Rice & Mango Salsa',
    ...
    return (<>
    <Navbar />
    <div className="overflow-hidden bg-gray-800 min-h-screen" style={{
        ...
        <div className='flex max-w-7xl mx-auto justify-center my-20 overflow-hidden'>
            <ul role="list" className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8 overflow-x-auto">
      {files.map((file) => ( 
        <li key={file.source} className="relative lg:mx-auto">
          <div className="group h-72 aspect-w-10 aspect-h-7 block w-full overflow-hidden rounded-lg bg-gray-100 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 focus-within:ring-offset-gray-100 max-h-52 md:max-h-80">
            <Image src={file.source} alt="" className=" object-cover group-hover:opacity-75" width={320} height={320}  /> 
            <button type="button" className="absolute inset-0 focus:outline-none">
              <span className="sr-only">View details for {file.title}</span>
            </button>
          </div>
          <p className=" mt-2 block truncate text-xs sm:text-sm text-center font-medium text-gray-50">{file.title}</p>
          <a href={file.size} className="pointer-events-none block text-xs sm:text-sm font-medium truncate md:whitespace-normal text-center text-gray-400">{file.size} </a>
        </li>
      ))}
    </ul>
        </div>
