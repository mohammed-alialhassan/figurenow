...
import Step2 from "../components/calorieCounter/step2";
import Step3 from "../components/calorieCounter/step3";
import Step4 from "../components/calorieCounter/step4";
...
export default function CalorieCounter() {
    ...
    const [ age, settingAge ] = useState({});
    const [ weight, settingWeight ] = useState({});
  ...
  const setGender = (value) => {
    setInfo({"gender": value});
    
  }

  const setAge = (value) => {
    settingAge({"age": value} );
    
  }

  const setWeight = (value) => {
    let kilograms = value * 0.45;
    settingWeight({"weight": kilograms})
  }

  const settingHeight = (value, value2) => {
    let feet = value;
    let inches = value2
  ...
  const settingActiveRate = (value) => {
    setActiveRate({"activeRate": value} );
  }

  const settingGoal = (value) => {
    setGoal({"goal": value} ); 
    setLoading(true);
  }

  const settingCals = (info, weight, height, age, goal, activeRate) => {
    if (info.gender === "Male") {
        ...
        let netHeight = 1.850 * Math.round(height.height);
        let netage = 4.676 * Math.round(age.age);
        let calories = 655.1 + netWeight + netHeight - netage;
        let bmr = calories * activeRate.activeRate;
        let targetCalories = Math.round(bmr * goal.goal);
        setBose({"calories": targetCalories});
        console.log("target calorie: ", targetCalories);
  ...
  return (
    <>
      ...
      <main>
        ...
        >
        {counter === 1? (<Step1 setGender={setGender} />) : counter === 2? (<Step2 setAge={setAge} />) : counter === 3? (<Step3 setWeight={setWeight} settingHeight={settingHeight} />) : counter === 4? (<Step4 settingActiveRate={settingActiveRate} />) : counter === 5? (<Step5 settingGoal={settingGoal} />) : counter === 6 && loading? (<Loading info={info} age={age} weight={weight} height={height} activeRate={activeRate} goal={goal} settingCals={settingCals} />) : counter === 6 && !loading? (<CalorieDisplay info={info} age={age} weight={weight} height={height} activeRate={activeRate} goal={goal} />) : (<div></div>)}
